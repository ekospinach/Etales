define(['directives', 'services'], function(directives){
    ...
    directives.directive('marketRuralElecssoriesShopperSales', ['Label','SeminarInfo','$http','PeriodInfo','$q', function(Label, SeminarInfo, $http, PeriodInfo, $q){
        return {
            scope : {
                isPageShown : '=',
                isPageLoading : '=',
            ...
            restrict : 'E',
            templateUrl : '../../partials/singleReportTemplate/MR_ruralElecssoriesShopperSales.html',                        
            link : function(scope, element, attrs){                                                                
                ...
                var loadTotal=function(data,ownerID,category,market){
                    ...
                    if(Values!=undefined){
                        var bmValue=Values.segmentInfo[4].shopperInfo[0].value.toFixed(2);
                        var onlineValue=Values.segmentInfo[4].shopperInfo[1].value.toFixed(2);
                        var mixedValue=Values.segmentInfo[4].shopperInfo[2].value.toFixed(2);
                    }else{
                        var bmValue=0;
                        var onlineValue=0;
                        var mixedValue=0;
                ...
                var loadMarketShopper=function(data,category,market){
                    for(var i=0;i<data.absoluteValue.length;i++){
                        if(data.absoluteValue[i].parentCategoryID==category&&data.absoluteValue[i].marketID==market){
                            ...
                            var bmValue=data.absoluteValue[i].segmentInfo[4].shopperInfo[0].value.toFixed(2);
                            var onlineValue=data.absoluteValue[i].segmentInfo[4].shopperInfo[1].value.toFixed(2);
                            var mixedValue=data.absoluteValue[i].segmentInfo[4].shopperInfo[2].value.toFixed(2);
                ...
                var organiseArray = function(data){
                    ...
                    loadTotal(data,1,1,2);
                    loadTotal(data,2,1,2);
                    loadTotal(data,3,1,2);
                    loadTotal(data,4,1,2);
                    loadTotal(data,5,1,2);
                    loadTotal(data,6,1,2);
