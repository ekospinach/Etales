        ...
        $http({
            ...
            for(var j=0;j<periodsShown.length;j++){
                for(var i=0;i<data.data.f_ShoppersShare.length;i++){
                    if(data.data.f_ShoppersShare[i].period==periodsShown[j]){

                        allChartsData.forEach(function(singleChartData){
                            if ((singleChartData.searchKeys.shopperKind == data.data.f_ShoppersShare[i].shopperKind) 
                                && (singleChartData.searchKeys.marketID == data.data.f_ShoppersShare[i].marketID)
                                && (singleChartData.searchKeys.categoryID == data.data.f_ShoppersShare[i].categoryID)){

                                singleChartData.chartSeries[data.data.f_ShoppersShare[i].storeID - 1].data.push(data.data.f_ShoppersShare[i].value * 100);
                            }
                        });
                    }
                }
