define(['app'], function(app) {
		app.controller('producerDecisionStep3Ctrl',
			...
			var getBrandMoreInfo=function(brandID,brandName){
				...
				var url='/brandHistoryInfo/'+SeminarInfo.getSelectedSeminar().seminarCode+'/'+(PeriodInfo.getCurrentPeriod()-1)+'/'+brandName;
				$http({method: 'GET', url: url})
				.then(function(data) {
					$scope.brandHistory=data.data;
					url="/producerBrandDecision/"+parseInt(PlayerInfo.getPlayer())+'/'+(PeriodInfo.getCurrentPeriod()-1)+'/'+SeminarInfo.getSelectedSeminar().seminarCode+'/'+brandName;
