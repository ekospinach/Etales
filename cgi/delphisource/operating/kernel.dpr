...
//        // WinningCriteriaWeights : ?
   ...
   var
    ReturnCode : LongWord;
   begin
    ReturnCode :=  RunOnePeriod( conf, period );
    if ( ReturnCode = kernel_SimulationRun_OK  ) then
    begin
      sResponseData.add('get kernel result complete, run time code:' + IntToStr( ReturnCode ));
      Log('get kernel result complete, run time code:' + IntToStr( ReturnCode ));
      Result := 'Status: 200 OK';
    end
    else
    begin
      sResponseData.add('get kernel result failed, run time code:' + IntToStr( ReturnCode ));
      Log('get kernel result failed, run time code:' + IntToStr( ReturnCode ));
...
begin
  ...
  try
    ...
    begin
      ...
      Log('Choosen period:' + IntToStr(vChoosenPeriod) + ', run kernel dll...');
      Writeln(getKernelResult(currentConf, vChoosenPeriod));
    end;
    Writeln;
